.PHONY: help build up down restart logs shell composer install migrate seed test clean setup-ssl setup-full setup-hosts remove-hosts
...
setup: ## Initial setup (install dependencies, generate key, migrate) - requires containers to be running
	docker-compose exec app composer install
	docker-compose exec app php artisan key:generate
	docker-compose exec app php artisan migrate
...
setup-full: ## Complete setup (hosts, SSL, build, up, install, key, migrate)
